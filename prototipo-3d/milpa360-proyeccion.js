...
(function(){
  ...
  window.MILPA_PROYECCION=function({tipo,entrar,fotograma,salir,pausar}){
    let chapters=MILPA_GUION[tipo];const audio=new Audio();audio.preload='auto';audio.volume=.85;
    ...
    const state={activo:false,reproduciendo:false,indice:0,tiempo:0,voz:true,bucle:false,terminado:false,audioListo:false,pitch:false};
    let token=0,prevFocus=null,subtitle='',ready=false;
    function info(){return window.MILPA_AUDIO?.[(state.pitch?'pitch':chapters[state.indice].id)+'-'+MILPA_I18N.lang];}
    function duracion(){if(state.pitch)return info().capitulos[state.indice].fin;return Math.max(chapters[state.indice].min,(info()?.duracion||25)+2);}
    function controls(){
      ...
      $('cine-note').textContent=state.pitch?pick('Concepto en evaluación · voz sintética · personas: Microsoft Rocketbox (MIT)','Concept under evaluation · synthetic voice · people: Microsoft Rocketbox (MIT)'):pick('Voz sintética · concepto en evaluación · Escape para salir','Synthetic voice · concept under evaluation · Escape to exit');
    }
    function audioError(){
    ...
    function cargarAudio(){
      ...
      audio.src='audio/'+(state.pitch?'pitch':chapters[state.indice].id)+'-'+MILPA_I18N.lang+'.mp3';audio.load();audio.currentTime=state.tiempo;playAudio();
    }
    async function capitulo(index,continuar=false){
      state.indice=Math.max(0,Math.min(chapters.length-1,index));state.tiempo=continuar?state.tiempo:state.pitch?info().capitulos[state.indice].inicio:0;state.terminado=false;subtitle='';ready=false;
      const n=++token;if(!continuar)audio.pause();$('cine-alert').textContent='';
      ...
      controls();
      try{await entrar(c.id,state.indice,{pitch:state.pitch});}catch(e){if(n!==token)return;$('cine-alert').textContent=pick('No se pudo preparar esta vista. Puedes avanzar al siguiente capítulo.','This view could not be prepared. You can move to the next chapter.');console.error(e);}
    ...
    return {state,audio,start,stop,pause,go:capitulo,duracion,tick(dt){
      ...
      const t=state.tiempo,d=duracion(),inicio=state.pitch?info().capitulos[state.indice].inicio:0,local=t-inicio,progreso=local/(d-inicio);
      fotograma?.(chapters[state.indice].id,progreso,state.reproduciendo?Math.min(dt,.1):0,local);
      if(state.pitch)$('cine-count').textContent=pick('Pitch','Pitch')+' · '+Math.ceil(Math.max(0,info().duracion-t))+' s · '+(state.indice+1)+' / '+chapters.length;
      ...
      const bar=$('cine-progress').children[state.indice]?.firstElementChild;if(bar)bar.style.width=Math.min(100,progreso*100)+'%';
      if(t>=d&&state.reproduciendo){if(state.indice<chapters.length-1)capitulo(state.indice+1,state.pitch);else if(state.bucle)capitulo(0);else {state.terminado=true;pause();}}
    }};
  };
})();
