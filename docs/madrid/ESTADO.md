...
**Sesión:** S0 y S1/P3 con checkpoint local; S2 en curso. **Fecha:** 20 sep 2026.
**Rama:** `preparacion-madrid-p1`. Base `0177a7a`; S0 `25f39e4`; P0 `7ea6b3f`, P1 `508f30a`, P2 `e8ffad4`.
...
## Resultado S1 y siguiente acción exacta

`milpa360-modelo.js` separa el estado del render. `milpa360_p3.py` exporta datos de P1/P2.
La configuración agrega supuestos ilustrativos identificados. `P3-SIMULACION.md` especifica
fronteras, ecuaciones, estados y límites. `verificar_p3.cjs` pasa: determinismo, pausa,
horizonte sin reinicio, cuatro fallos, autorización segura, cuarentena y conservación.
Los 12 cultivos precargados no demuestran arranque; el banco comparador usa parcelas fijas
con igual biomasa inicial, agua y electricidad. El daño permanece tras reparar, y cada
estrategia se compara con su propio control nominal. No existe porcentaje ganador fijo.
El inventario de gas recibe una entrada nominal P1: no simula la digestión durante fallos.

**Siguiente:** S2, prueba de integración en Chrome, controles de fallos/reinicio y vista técnica,
Three.js local único, capturas vigentes y paquete sin Internet. S1 no acredita todavía la
integración visual ni el equipo de presentación. Después, S3/P4 para mecánica y ensayos,
incluido B19. No publicar ni hacer push en esta ejecución.
